...
| Negative quantities | 30 | Flagged as likely returns (`is_return`), then converted to absolute values | Sign was a data-entry error; flag preserves business signal for downstream use |
...
A fourth limitation is that **negative quantities were treated as sign errors rather than returns.** The `is_return` flag was preserved, but the actual `.abs()` conversion means these 30 orders are counted as forward sales in all revenue calculations. If some were genuine returns, the total revenue figure is slightly inflated. Without a transaction-type column, there is no way to separate them accurately.
